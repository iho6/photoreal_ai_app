...
# Comfy CLIPLoader (type=flux2) needs this single-file TE — not HF shards.
HF_COMFY_QWEN_REPO = "Comfy-Org/vae-text-encorder-for-flux-klein-9b"
HF_COMFY_QWEN_FILE = "split_files/text_encoders/qwen_3_8b.safetensors"
QWEN_CLIP_NAME = "qwen_3_8b.safetensors"
QWEN_CLIP_MIN_BYTES = 1_000_000_000
...
def _write_local_comfy_paths() -> None:
    from photoreal.portal.paths import write_comfy_extra_local_yaml

    out = write_comfy_extra_local_yaml(REPO_ROOT)
    print(f"Wrote {out}")
...
def download_photoreal_gen(
    *,
    ...
    te_flat = KLEIN_DIR / "text_encoder" / QWEN_CLIP_NAME
    print("[photoreal_gen] Comfy-Org Qwen3-8B CLIP (single-file for CLIPLoader)...")
    try:
        tqdm_cls = _enable_portal_hf_progress()
    except ImportError as e:
        raise SystemExit(
    ...
    records["assets"].append(
        _hf_download_to(
            repo_id=HF_COMFY_QWEN_REPO,
            filename=HF_COMFY_QWEN_FILE,
            dest=te_flat,
            token=_hf_token(),
            tqdm_cls=tqdm_cls,
            min_bytes=QWEN_CLIP_MIN_BYTES,
        )
    )
    records["notes"].append(
        "Comfy CLIPLoader uses Comfy-Org "
        f"{HF_COMFY_QWEN_FILE} as {QWEN_CLIP_NAME} "
        "(HF text_encoder shards alone are not enough)."
    )
